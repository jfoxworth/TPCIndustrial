<section id="section-about" class="page-section">

	<div class="container clearfix">


		<div class="heading-block center">
			<h2>TPC Industrial</h2>
			<span>A Leader in the PVF / Hose & Fitting Industry</span>
		</div>


		<div class="col_one_third nobottommargin">
			<div class="feature-box media-box">
				<div class="fbox-media">
					<img src="/images/square0.jpg" alt="Why choose Us?" width="200px">
				</div>
				<div class="fbox-desc">
					<h3>Who We Are.<span class="subtitle">A Leader in the PVF Industry.</span></h3>
					<p>TPC INDUSTRIAL, LLC is a major stocking distributor of pipe, valve, fittings, flanges, bolts, gaskets, hose, hose assemblies, CAM and groove couplings and accessories.</p>
				</div>
			</div>
		</div>

		<div class="col_one_third nobottommargin">
			<div class="feature-box media-box">
				<div class="fbox-media">
					<img src="/images/square4.jpg" alt="Why choose Us?" width="200px">
				</div>
				<div class="fbox-desc">
					<h3>What We Do.<span class="subtitle">Service is our Priority.</span></h3>
					<p>When our customers need it, we make sure they have it, on time every time, day or night 24/7. We make it happen when others can't. "Service is our Priority".</p>
				</div>
			</div>
		</div>

		<div class="col_one_third nobottommargin col_last">
			<div class="feature-box media-box">
				<div class="fbox-media">
					<img src="/images/square3.jpg" alt="Why choose Us?" width="200px">
				</div>
				<div class="fbox-desc">
					<h3>Where We Are.<span class="subtitle">Convenient Pasadena Location.</span></h3>
					<p>Our location in Pasadena, Texas gives us quick and easy access to HWY 225, allowing for quick service to the greater Houston area and beyond.</p>
				</div>
			</div>
		</div>

		<div class="clear"></div>

	</div>

</section>

